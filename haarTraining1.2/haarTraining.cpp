...
//		str = "begin";
//		while (str.size()>0)   //按行读取,遇到换行符结束
		while(getline(istream, str))
		{
	//		one = clock();
	//		getline(istream, str);
	//		two = clock();
	//		cout << "读入一行：" << (two - one) / CLOCKS_PER_SEC * 1000 << endl;
...
//			five = clock();
	...
	//		six = clock();
	//		cout << "弱分类器训练：" << (six - five) / CLOCKS_PER_SEC * 1000 << endl;
			if (i == 0)
			{
			...
			i++;
	//		double seven = clock();
	//		cout << "当前计算完：" << (seven - one) / CLOCKS_PER_SEC * 1000 << endl;
		}
